...
def message_type_specify(message_type, time_diff, j):
    if time_diff > 96:
        message_type = f'backfill-data-{j}'
    else:
        message_type = f'live-data-{j}'
        print(message_type)
    return message_type
...
def on_message(client, userdata, msg):
    ...
    for i in range(len(incoming_data['values'])):
        ...
        data['message_type'] = message_type_specify(
            data['message_type'], time_difference(data['date'], data['createdTime']),j
        )
        ...
        print(data)
        producer.send('inkai-frozendata9', value=data)    
    j = j - 1
