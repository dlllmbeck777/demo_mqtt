...
def write_influx(data,send_time):
    ...
    p = influxdb_client.Point(data["payload"]["insert"][0]["fqn"]).tag("version", data["header"]["version"]).tag("message_type", "back-data-1").tag("layer", data["header"]["layer"]).field("quality", data["payload"]["insert"][0]["vqt"][0]["q"]).field("tag_value", data["payload"]["insert"][0]["vqt"][0]["v"]).time(send_time)
