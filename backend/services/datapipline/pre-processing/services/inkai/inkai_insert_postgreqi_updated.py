...
def updated_value_table(row_line_new):
    row_line = row_line_new
    ...
    for measure in row_line:
        ...
        dict_res = {
            'measurement':str(measure.get('measurement'))
            ,'short_name':str(measure.get('short_name'))
            ,'asset':str(measure.get('asset'))
            ,'layer':str(measure.get('layer'))
            ,'tag_value':str(measure.get('tag_value'))
            ,'tag_quality':str(measure.get('tag_quality'))
            }

        query = '''
            update change_widgets_timerange_v2
            set short_name = '{}'
                ,asset = '{}'
                ,layer = '{}'
                ,tag_value = '{}'
                ,tag_quality = '{}'
                ,inserted = now()
            where lower(measurement) = lower('{}')
            '''.format(dict_res.get('short_name'),dict_res.get('asset'),dict_res.get('layer'),dict_res.get('tag_value'),dict_res.get('tag_quality'),dict_res.get('measurement'))
...
#updated_value_table('xxx')

for line in sys.stdin:
    #print('Line: ',line)
    processed_data = updated_value_table(line.strip())
