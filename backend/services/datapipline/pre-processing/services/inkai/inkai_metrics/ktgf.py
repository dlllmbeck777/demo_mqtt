...
def applyktgf(event_type):
    ...
    cursor.execute(
        'SELECT "ITEM_ID", "START_DATETIME", "VAL24", "VAL25", "id" FROM item_event_item_event WHERE "EVENT_TYPE" = %s',
        (event_type,),
    )
    ...
    for item_id, start_datetime, mtw, mttm, id in comp_read_results:
        try:
            year = datetime.utcfromtimestamp(start_datetime / 1000).year
            average_maintenance_duration = average_maintenance_duration_per_year.get(
                year, 0
            )
            ktgf = mttm / (mttm + average_maintenance_duration + mtw)
            cursor.execute(
                'UPDATE item_event_item_event SET "VAL32" = %s WHERE "id" = %s ',
                (ktgf, id),
            )
        except Exception as e:
            continue
