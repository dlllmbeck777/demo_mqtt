...
def applymtbf(event_type):
    ...
    cur.execute(
        "SELECT \"ITEM_ID\", \"START_DATETIME\" FROM item_event_item_event WHERE \"EVENT_TYPE\" = 'DOWNTIME' AND \"CHAR1\" NOT IN ('L4-DO-P-MI_PUM_P1','L4-DO-P-MI_COM_P1','L4-DO-P-MI_COM_P2','L4-DO-P-MI_COM_P3','L4-DO-P-MI_COM_P4')"
    )
    ...
    for item_id, start_datetime, id in comp_read_results:
        ...
        if downtime_count > 0:
            ...
            if total_operating_time:
                ...
                update_query = (
                    'UPDATE item_event_item_event SET "VAL21" = %s WHERE "id" = %s'
                )
                cur.execute(update_query, (mtbf, id))
                conn.commit()
        else:
            update_query = (
                'UPDATE item_event_item_event SET "VAL21" = 0 WHERE "id" = %s'
            )
