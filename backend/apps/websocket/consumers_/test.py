...
from utils.consumer_utils import find_tag, retive_influx_live_data
import os
import environ
import gzip
...
class TestConsumer(AsyncWebsocketConsumer):
    async def send_messages(self):
        while self.is_active:
            # print(self.offset)
            query_tuple = retive_influx_live_data(**self.kwargs)
            ...
            if data:
                ...
                for item in new_data:
                    # print(list(item.values())[0][1][0][0])
                    new_list.append(
                        [
                            list(item.values())[0][1][0][0],
                            list(item.values())[0][1][0][1],
                        ]
                    )
