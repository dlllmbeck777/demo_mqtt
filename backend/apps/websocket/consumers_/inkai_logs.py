import datetime
import threading
from channels.generic.websocket import AsyncWebsocketConsumer
import asyncio
import json
import time
import redis
from asgiref.sync import sync_to_async, async_to_sync
from utils.consumer_utils import retrive_mongo_notifications
import os
import environ
import gzip
from datetime import datetime, timedelta
from channels.exceptions import StopConsumer

env = environ.Env(DEBUG=(bool, False))


class WSInkaiLogsConsumer(AsyncWebsocketConsumer):
    async def last_messages(self):
        try:
            while self.is_active:
                current_time = datetime.now()

                one_hour_ago = current_time - timedelta(hours=int(self.get_hours))
                timestamp = int(one_hour_ago.timestamp()) * 1000
                try:
                    query = {
                        "layer": self.kwargs["layer"],
                        "time": {"$gt": timestamp},
                    }
                    kwargs = {
                        "db_name": self.kwargs["layer"],
                        "collections": "logs",
                        "query": query,
                    }
                    data = retrive_mongo_notifications(**kwargs)
                    if data:
                        if self.is_active:
                            try:
                                await self.send(json.dumps((data)))
                            except:
                                print("HATA YAKALNDI, logs")
                                raise StopConsumer

                    await asyncio.sleep(5)
                except Exception as e:
                    print(e)
        except asyncio.CancelledError:
            self.is_active = False

    async def connect(self):
        await self.accept()
        db_name = "logs"  # os.environ["MongoDb_alarms_Name"]
        layer_name = self.scope["url_route"]["kwargs"]["layer_name"]
        self.is_active = True
        self.kwargs = {"db_name": db_name, "layer": layer_name}
        self.task = None
        # self.task = asyncio.create_task(self.send_messages())

    async def receive(self, text_data):
        try:
            self.get_hours = json.loads(text_data)
            if self.task is not None:
                self.task.cancel()
            self.task = asyncio.create_task(self.last_messages())
        except Exception as e:
            print(str(e))
            pass

    async def disconnect(self, close_code):
        try:
            self.is_active = False
            self.task.cancel()
            print("disconnect", close_code)
        except BaseException as e:
            print(e)

    async def disconnect(self, close_code):
        try:
            self.is_active = False
            self.task.cancel()
            print("disconnect", close_code)
        except BaseException as e:
            print(e)
