...
import { handleChange } from "../../../services/actions/feedback/feedback";
import "../../../assets/styles/page/administration/profile/personalInfo.scss";
const Helper = ({ value, ...rest }) => {
  const dispatch = useDispatch();
  const err = useSelector(
    (state) => state.feedback.errors?.[value.toUpperCase()]
  );
  const val = useSelector(
    (state) => state.feedback.values?.[value.toUpperCase()]
  );
  ...
  return (
    <Box sx={{ position: "relative" }}>
      ...
      <TextField
        ...
        type={"text"}
        onChange={(event) => {
          dispatch(handleChange(value.toUpperCase(), event.target.value));
        }}
        value={val}
        error={err}
        {...rest}
