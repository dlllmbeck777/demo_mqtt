          ...
          if (
            ...
            Promise.all(
              Object.keys(res.data).map((e) => {
                if (res.data[e]?.data?.[0]?.rms_fft_frequency)
                  Promise.all(
            ...
            let res = await TagService.getRms(body, layer);
            updateXAxisTitle(series, time);
            helperUpdateAfterUploadData(series);
            ...
            Promise.all(
              Object.keys(dataList).map((e) => {
                Promise.all(
                  res.data[e]?.data?.map((data, i) => {
                    dataList[e].push([data.time, data.rms]);
                  })
                );
              })
            );
            ...
            const body = JSON.stringify({
              TAGS: Object.keys(dataList),
              TYPE: "accleration_peak_factor_new",
            });
            let res = await TagService.getFft(body, layer);
            updateXAxisType(series, "datetime");
            updateXAxisTitle(series, time);
            ...
            const body = JSON.stringify({
              TAGS: Object.keys(dataList),
              TYPE: "accleration_high_frequency",
            });
            let res = await TagService.getFft(body, layer);
